 ...
 * <p>
 * To ensure proper use of this class, it is highly reccomended that the {@link } class is used, since it handles all of these
...
public class PerformancePlotter extends JFrame implements EnvironmentObserver {
	...
	public PerformancePlotter(String firstAgentName, int chartWidth, int chartHeight, int columns, int maxWindowHeight,
		...
		this.agentTrials.put(this.curAgentName, new ArrayList<Trial>());
	...
	@Override
	synchronized public void environmentStep(Pair<RLState,Double> result) {
		if(!this.collectData){
			return;
		}

		this.curTrial.stepIncrement(result.getSecond());
		this.curTimeStep++;

	}

	@Override
	public void environmentReset(RLEnvironment resetEnvironment) {
		//do nothing
		endEpisode();
	}
		...
		SwingUtilities.invokeLater(new Runnable() {
			...
			public void run() {
				...
				synchronized (PerformancePlotter.this) {
					...
					PerformancePlotter.this.agentTrials.put(PerformancePlotter.this.curAgentName, new ArrayList<Trial>());
